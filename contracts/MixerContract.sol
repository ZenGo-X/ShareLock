...
contract MixerContract {
    ...
    function payOutCleanCoins(address A, address[] memory P, uint8 v, bytes32 r, bytes32 s) public {
        ...
        require(sessions[A].senderAddresses.length == P.length, "Not all participants deposited!");
        ...
        uint totalAmt = sessions[A].arrivedAmount;
        sessions[A].arrivedAmount = 0;
        for(uint i=0; i < P.length; i++) {
            address(uint160(P[i])).transfer(amt);
            delete sessions[A].senderAddresses[i];
        }
        if (totalAmt-P.length*amt > 0) {
            msg.sender.transfer(totalAmt-P.length*amt); //any surplus is provided to the one who sends the tx
        }
    }
    ...
    function payBackDirtyCoins(address A, uint index) public {
        ...
        delete sessions[A].senderAddresses[index]; // saving gas by erasing mapping items
    }
}